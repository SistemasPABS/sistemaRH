...
if($campo != NULL){
    switch ($parametro){
        case 'nom':
            ...
            break;
        case 'cve':
            $where = "where persona_cve like '%$campo%'";
            break;
    }
}else{
    $where=" ";
...
$sqlxls="select * from vw_personas ".$where." order by persona_cve desc";
...
if($extension == 'xls'){
    ...
    if($rowxls=pg_fetch_array($resultxls)){
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('D1', 'Tel. fijo')
                ->setCellValue('E1', 'Celular')
                ->setCellValue('F1', 'Activo');
        do{
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A'.$a, $rowxls['persona_cve'])
                    ->setCellValue('B'.$a, $rowxls['nombrecompleto'])
                    ->setCellValue('C'.$a, $rowxls['persona_correo'])
                    ->setCellValue('D'.$a, $rowxls['persona_tel_fijo'])
                    ->setCellValue('E'.$a, $rowxls['persona_tel_movil'])
                    ->setCellValue('F'.$a, $rowxls['persona_status']);
                    $a++;
        }
