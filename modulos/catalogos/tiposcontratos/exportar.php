...
if($campo != NULL){
    switch ($parametro){
        case 'nom':
            $where = "where tipoc_nombre like '%$campo%'";
            break;
        case 'cve':
            $where = "where tipoc_cve = $campo";
            break;
    }
}else{
    $where=" ";
...
$sqlxls="select * from vw_tipos_contratos $where order by tipoc_id desc";
...
if($extension == 'xls'){
    ...
    if($rowxls=pg_fetch_array($resultxls)){
        ...
        do{
            $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A'.$a, $rowxls['tipoc_id'])
                ->setCellValue('B'.$a, $rowxls['tipoc_cve'])
    ...
    // Rename worksheet*/
    $objPHPExcel->getActiveSheet()->setTitle('Tipos de contratos');
