...
$sqlxls="select * from sucursales $where order by suc_id desc";
...
if($extension == 'xls'){
    ...
    if($rowxls=pg_fetch_array($resultxls)){
        ...
        do{
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A'.$a, $rowxls['suc_id'])
                    ->setCellValue('B'.$a, $rowxls['plaza_id'])                    
                    ->setCellValue('C'.$a, $rowxls['suc_nombre'])
                    ->setCellValue('D'.$a, $rowxls['suc_activo']);
                    $a++;
        }
