...
class creacomisiones extends conectasql{
    protected $usid;
    protected $em;
    ...
    public function interfaz() {
        echo '<form name="opbusqueda" id="opbusqueda" method="post" action="exportar.php" target="_blank" >';
            ...
            echo '<div name="busqueda" id="busqueda">';
                ...
                //print_r($this->p3);
                if(in_array(26, $this->p3)){echo '<input class="cierre2" type="button" name="nuevo"    id="nuevo"    value="Nuevo"     onclick="popup(\'administrar/nuevo_editar_com.php\',\''. base64_encode($this->em).'\',\''. base64_encode('nuevo').'\');"  style="width:45px;" > ';}
                if(in_array(27, $this->p3)){echo '<input class="cierre2" type="button" name="editar"   id="editar"   value="Editar"    onclick="edita(\'administrar/nuevo_editar_com.php\',\''. base64_encode($this->em).'\',\''. base64_encode('editar').'\');" style="width:40px;" > ';}
                if(in_array(28, $this->p3)){echo '<input class="cierre2" type="button" name="eliminar" id="eliminar" value="Eliminar"  onclick="eliminar_r(\'administrar/eliminar.php\');"   style="width:50px;" > ';}
                if(in_array(29, $this->p3)){echo '<input class="cierre2" type="button" name="export"   id="expxls"   value="XLS"       onclick="exportar(\'exportar.php\',\'xls\');"               style="width:40px;" > ';}
            echo '</div>';  
        $this->listado();
        echo '</form>';
    }
