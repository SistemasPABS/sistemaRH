...
if($campo != NULL){
    switch ($parametro){
        case 'nom':
            $where = "where raz_nombre like '%$campo%'";
            break;
        case 'cve':
            $where = "where raz_id = $campo";
            break;
    }
}else{
    $where=" ";
...
$sqlxls="select * from vw_razones $where order by raz_id desc";
...
if($extension == 'xls'){
    ...
    if($rowxls=pg_fetch_array($resultxls)){
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('D1', 'Representante');
        do{
            $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A'.$a, $rowxls['raz_id'])
                ->setCellValue('B'.$a, $rowxls['raz_nombre'])
                ->setCellValue('C'.$a, $rowxls['raz_direccion'])
                ->setCellValue('D'.$a, $rowxls['raz_legal']);
                $a++;
        }
    ...
    // Rename worksheet*/
    $objPHPExcel->getActiveSheet()->setTitle('Razones Sociales');
