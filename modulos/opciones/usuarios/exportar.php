...
if($campo != NULL){
    switch ($parametro){
        case 'nom':
            ...
            break;
        case 'log':
            $where = "where us_login like '%$campo%'";
            break;
    }
}else{
    $where=" ";
...
$sqlxls="select * from vw_users $where order by us_id desc";
...
if($extension == 'xls'){
    ...
    if($rowxls=pg_fetch_array($resultxls)){
        ...
        do{
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A'.$a, $rowxls['us_login'])
                    ->setCellValue('B'.$a, $rowxls['nombrecompleto'])
                    ->setCellValue('C'.$a, $rowxls['us_activo'])
                    ->setCellValue('D'.$a, $rowxls['correo']);
                    $a++;
        }
